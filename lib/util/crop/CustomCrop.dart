...
enum _CropAction { none, moving, cropping, scaling, movingCropArea }
...
class CropState extends State<Crop> with TickerProviderStateMixin, Drag {
  ...
  _CropHandleSide _hitCropHandle(Offset localPoint) {
    ...
    if (viewRect.contains(localPoint)) {
      //debugPrint("center move");
      return _CropHandleSide.center;
    }

    // if (Rect.fromLTWH(
    //   ((viewRect.left+viewRect.right)*0.5) - _kCropHandleHitSize / 2,
    //   ((viewRect.top+viewRect.bottom)*0.5) - _kCropHandleHitSize / 2,
    //   _kCropHandleHitSize,
    //   _kCropHandleHitSize,
    // ).contains(localPoint)) {
    //   //debugPrint("center move");
  ...
  void _updateArea(
    ...
    if (cropHandleSide != _CropHandleSide.centerH &&
        cropHandleSide != _CropHandleSide.centerV) {
    ...
    if (cropHandleSide == _CropHandleSide.centerH ||
        cropHandleSide == _CropHandleSide.centerV) {
      //debugPrint("center Vertical " + areaRight.toString());
      //debugPrint("center Horizontal " + areaLeft.toString());
      ...
      if (areaLeft <= 0.0) {
        areaRight += areaLeft.abs();
        areaLeft = 0;
      }
      if (areaRight >= 1.0) {
        areaLeft -= areaRight - 1.0;
        areaRight = 1.0;
      }

      if (areaTop <= 0.0 || areaBottom >= 1.0) {
        if (areaTop <= 0.0) {
          areaTop = 0.0;
        }
  ...
  void _handleScaleUpdate(ScaleUpdateDetails details) {
    if (_action == _CropAction.none) {
      ...
      } else if (_handle == _CropHandleSide.center) {
        _action = _CropAction.movingCropArea;
      } else {
        _action = _CropAction.cropping;
      }
    }

    if (_action == _CropAction.movingCropArea) {
      ...
      if (dy > dx) {
        _updateArea(
          left: dx,
          top: dy,
          cropHandleSide: _CropHandleSide.centerH,
        );
        _updateArea(
            bottom: dy, right: dx, cropHandleSide: _CropHandleSide.centerH);
      }

      if (dx > dy) {
        _updateArea(
            right: dx, top: dy, cropHandleSide: _CropHandleSide.centerV);
        _updateArea(
            left: dx, bottom: dy, cropHandleSide: _CropHandleSide.centerV);
      }
    } else if (_action == _CropAction.cropping) {
      final delta = details.focalPoint - _lastFocalPoint;
      _lastFocalPoint = details.focalPoint;
